...
def get_email_or_phone(txt_lists, index, n, write_file):
    """
    ...
    with open(write_file, 'a', encoding='utf-8', errors='ignore')as wf:
        for v in email_dict.values():
            v_length = len(v)
            if v_length >= n:  # 用于去掉单个（或者重复次数有要求）的记录
                for s in v:
...
def get_original_dict_process():
    ...
    :return:
    """
    txt_lists = get_all_path(r'E:/data4')  # 获取符合要求的txt文件
    # dir = r"E:/data/"
    # txt_process(txt_lists, dir)  # 原始文件的二次处理，进一步删除不符合要求的信息,保存目录dir

    # index邮箱4，电话5，改动这个参数即可；n是保留的重复数
    index = 5
    n = 4
    # write_file = r'E:\data5_res\email_data_4.txt'  # 邮箱
    # write_file = r'E:\data5_res\phone_data_4.txt' #手机
    # get_email_or_phone(txt_lists, index, n,write_file)

    # email_username_file = r'E:\data4_res\email_username.txt'
    # get_email_username(txt_lists, email_username_file)

    # top = 100  # 取前top个
    # email_original_top = r'E:\data5_res\email_top100.txt'
    # get_original_dict_count(txt_lists, top, email_original_top)

    # 前top的原始信息
    # read_file = r'E:\data5_res\email_top100.txt'
